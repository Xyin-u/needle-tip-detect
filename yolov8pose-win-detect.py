...
def draw_results(img, dets, gts, output_path, show_coords=True, show_conf=True):
    ...
    for gt in gts:
        for (x, y) in gt['keypoints']:
            if x != 0 or y != 0:
                ...
                if show_coords:
                    text = f"({int(x)},{int(y)})"
                    cv2.putText(img_copy, text, (int(x) + 5, int(y) - 5),
    ...
    for det in dets:
        ...
        if show_conf:
            ...
            if text_y < 5:
                text_y = y1 + 15
            cv2.putText(img_copy, conf_text, (text_x, text_y),
        ...
        for (x, y, conf_kpt) in det['keypoints']:
            if conf_kpt > 0.3 and (x != 0 or y != 0):
                ...
                if show_coords:
                    text = f"({int(x)},{int(y)})"
                    cv2.putText(img_copy, text, (int(x) + 5, int(y) + 20),
...
def main():
    ...
    stats_rows = pd.DataFrame([
        {"文件名": "平均误差", "X坐标": "", "Y坐标": "", "是否有针尖": "", "X_ture": "", "Y_true": "", "距离": mean_err},
        {"文件名": "最大误差", "X坐标": "", "Y坐标": "", "是否有针尖": "", "X_ture": "", "Y_true": "", "距离": max_err},
        {"文件名": "误检个数", "X坐标": "", "Y坐标": "", "是否有针尖": "", "X_ture": "", "Y_true": "", "距离": false_positive_count},
        {"文件名": "漏检个数", "X坐标": "", "Y坐标": "", "是否有针尖": "", "X_ture": "", "Y_true": "", "距离": miss_count}
    ])
