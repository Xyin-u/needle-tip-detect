...
class WingLoss(nn.Module):
    def __init__(self, omega=10, epsilon=2):
        ...
        # 预计算常数 C = omega - omega * ln(1 + omega/epsilon)
...
class KeypointLoss(nn.Module):
    ...
    def __init__(self, sigmas: torch.Tensor) -> None:
        ...
        # # 添加 Wing Loss 实例 注释这两行
        # self.wing_loss = WingLoss(omega=10, epsilon=2)
        # # 选择ARobustLoss（超参数可调）
        # self.arobust_loss = ARobustLoss(alpha=2.0, c=1.0)
    ...
    def forward(
        ...
        # # # ========== 修改开始 ==========
        #
        # valid_mask = kpt_mask > 0   # 形状 (N, K)
        # if not valid_mask.any():
        #     return (pred_kpts * 0).sum()
        #
        # pred_valid = pred_kpts[valid_mask]      # (M, 2)
        # gt_valid = gt_kpts[valid_mask]          # (M, 2)
        # # 直接计算 Loss
        # # loss = self.wing_loss(pred_valid, gt_valid)
        # loss = self.arobust_loss(pred_valid, gt_valid)
        #
        # return loss
        # # # ========== 修改结束 ==========
