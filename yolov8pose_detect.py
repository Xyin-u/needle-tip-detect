...
labels_dir = Path(
    r"D:\xinyu\ultralytics-main\datasets\tip-traindata0601\labels\test"
)  # 存放真实标注的目录（YOLO格式txt）
...
for img_path in input_dir.glob("*.*"):
    ...
    if success and gt_visible:
        ...
        cv2.putText(img, f"GT({gt_x},{gt_y})", (gt_x + 5, gt_y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
        print(f"  像素误差: {dist:.2f} px (预测: {tip_x},{tip_y} 真实: {gt_x},{gt_y})")
    elif success and not gt_visible:
        print("  真实关键点无效(不可见)，无法计算误差")
    elif not success and gt_visible:
        print("  模型未检测到关键点，但真实存在 -> 漏检")
